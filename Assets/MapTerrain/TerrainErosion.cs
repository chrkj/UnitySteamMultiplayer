...
namespace MapTerrain
{
    public class TerrainErosion : MonoBehaviour
    {
        ...
        public void Run()
        {
            ...
            for (int i = 0; i < iterations - 1; i++)
            {
                ...
                for (int j = 0; j < 20; j++)
                {
                    ...
                    for (int _x = -1; _x <= 1; _x++)
                    {
                        for (int _z = -1; _z <= 1; _z++)
                        {
                            ...
                            if (coords.x + _x < 0 || coords.y + _z < 0 || coords.x + _x > heightmapSize - 1 ||
                                coords.y + _z > heightmapSize - 1 ||
                                previousCoords == new Vector2Int(coords.x + _x, coords.y + _z))
                            {
                                data.value = 8000;
                            }
                            else
                ...
                for (int _x = -depositWidth; _x <= depositWidth; _x++)
                {
                    for (int _z = -depositWidth; _z <= depositWidth; _z++)
                    {
                        ...
                        {
                            float falloff = 1 -
                                            Mathf.Clamp01(Vector2.Distance(new Vector2(_x, _z), Vector2.zero) /
                                                          depositWidth);
                            heightMap[coords.x + _x, coords.y + _z] +=
                                (mass * Mathf.Clamp01(massPerIteration) * falloff) / (depositWidth * 50f);
                        }
                    }
                }
            }
            ...
            terrain.terrainData.size = new Vector3(terrain.terrainData.size.x - 1, terrain.terrainData.size.y,
