...
namespace MapTerrain
{
    ...
    {
        [Header("Size")] public int width = 4000;
        [SerializeField] private float height = 250;

        [Header("Noise")] [SerializeField] private float scale = 5;
        ...
        [Range(0, 1)] [SerializeField] private float beachHeight = 0.35f;

        public float seed = 0;

        [Header("Objects")] [SerializeField] private TRNTerrainObject[] prefabs;
        [SerializeField] private TRNDetaiTexture[] detailTextures;
        [HideInInspector] public bool spawnPrefabs = true;

        [Header("Errode")] [SerializeField] private bool erode = true;
        public ComputeShader HydraulicErosionComputeShader;
        [SerializeField] private int h_Iterations = 100000;
        ...
        public void Generate()
        {
            ...
            for (int x = 0; x < heightSize; x++)
            {
                for (int z = 0; z < heightSize; z++)
                {
                    ...
                    {
                        noiseLayers.Add(TerrainNoise.LayeredNoise(structure.noiseType, uv, scale * structure.scale,
                            structure.octaves, structure.persistence, structure.lacunarity));
                    ...
                    for (int i = 0; i < noiseLayers.Count; i++)
                    {
                        ...
                        {
                            noiseLayer = noiseFilter.filterType switch
                            {
                                FilterType.Canyon => TerrainFilters.Canyons(noiseLayer, noiseFilter.threshold,
                                    noiseFilter.power),
                                FilterType.Exponential => TerrainFilters.Exponential(noiseLayer, noiseFilter.exponent),
                                FilterType.Invert => TerrainFilters.Invert(noiseLayer),
                                FilterType.Ridge => TerrainFilters.Ridge(noiseLayer),
                                FilterType.Smoothstep => TerrainFilters.Smoothstep(noiseLayer, noiseFilter.step,
                                    noiseFilter.smoothness),
                                _ => noiseLayer
                            };
                        }
                    ...
                    float riverMap = rivers ? TerrainNoise.RiverMap(new Vector2(x, z), riverScale) : 1f;
                    heightMap[x, z] = (noiseHeight / total) *
                                      TerrainFilters.Falloff(uv, heightSize, new Vector2(x, z), beachHeight) *
                                      riverMap;
                }
            }
        ...
        public void Smooth()
        {
            ...
            for (int x = 0; x < heightSize; x++)
            {
                for (int z = 0; z < heightSize; z++)
                {
                    ...
                    smoothedHeightmap[x, z] =
                        Mathf.Lerp(total, _heightMap[x, z], 1 - Mathf.Pow(1 - _heightMap[x, z], 8));
                }
            }
            ...
            for (int i = 0; i < prefabs.Length; i++)
            {
                ...
                for (int x = 0; x < width; x++)
                {
                    for (int z = 0; z < width; z++)
                    {
                        ...
                        {
                            float xOffset = (TerrainHeightmap.random(new Vector2(z + seed * i, x + seed * i)) * 2) - 1;
                            float zOffset = (TerrainHeightmap.random(new Vector2(x + seed * i, z + seed * i)) * 2) - 1;
                            ...
                            float _y = terrain.terrainData.GetHeight(
                                (int)(_x * terrain.terrainData.heightmapResolution),
                                (int)(_z * terrain.terrainData.heightmapResolution)) / (float)height;
                            float grad = terrain.terrainData.GetSteepness(_x, _z);

                            tree.position = new Vector3(_x, _y, _z);
                            tree.heightScale =
                                (TerrainHeightmap.random(new Vector2(z + seed, x + seed)) *
                                 (prefabs[i].sizeRange.y - prefabs[i].sizeRange.x)) + prefabs[i].sizeRange.x;
                            tree.widthScale = tree.heightScale;

                            tree.prototypeIndex = i;

                            if (_y > prefabs[i].heightRange.x / height && _y < prefabs[i].heightRange.y / height &&
                                grad < 20)
                            {
                                trees.Add(tree);
                            }
        ...
        public Vector3 DetailToWorld(int x, int y)
        {
            ...
            return new Vector3(
                ...
                (((float)x / (float)terrain.terrainData.detailWidth) * (terrain.terrainData.size.x)),
                0f,
                terrain.GetPosition().z +
                (((float)y / (float)terrain.terrainData.detailHeight) * (terrain.terrainData.size.z))
            );
        }
            ...
            for (int x = 0; x < terrain.terrainData.detailWidth; x++)
            {
                for (int z = 0; z < terrain.terrainData.detailWidth; z++)
                {
                    ...
                    for (int i = 0; i < detailTextures.Length; i++)
                    {
                        ...
                        int xPos = Mathf.Clamp(x + (int)Random.Range(-spacing, spacing), 1,
                            terrain.terrainData.detailWidth - 1);
                        int zPos = Mathf.Clamp(z + (int)Random.Range(-spacing, spacing), 1,
                            terrain.terrainData.detailWidth - 1);
                        if (wPos.y > detailTextures[i].heightRange.x && wPos.y < detailTextures[i].heightRange.y &&
                            grad < 20 && (x % detailTextures[i].spacing == 0 && z % detailTextures[i].spacing == 0))
                        {
    ...
    {
        [Header("Noise")] public NoiseType noiseType;
        public float scale = 1;
        public int octaves = 5;
        public float persistence = 0.5f;
        public float lacunarity = 2;
        [Range(0, 1)] public float weight = 1;
        [Header("Filters")] public NoiseFilter[] filters;
    }
    ...
    public class NoiseFilter
    {
        ...
        [Header("Exponential Settings")] public float exponent;

        [Header("Smoothstep Settings")] [Range(0, 1)]
        public float step;

        [Range(0, 2)] public float smoothness;

        [Header("Canyon Settings")] [Range(0, 1)]
