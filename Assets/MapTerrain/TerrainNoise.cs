...
namespace MapTerrain
{
    ...
    {
        public static float LayeredNoise(NoiseType noiseType, Vector2 uv, float scale, int octaves,
            float persistence = 2, float lacunarity = 0.5f)
        {
            uv *= scale;
            float Noise = 0;
            float strength = 1;
        ...
        public static float RiverMap(Vector2 uv, float scale)
        {
            ...
            largeNoise = TerrainFilters.Ridge(largeNoise);

            float smallNoise = Mathf.PerlinNoise(uv.x * 30, uv.y * 30);
            smallNoise = TerrainFilters.Ridge(smallNoise);

            float blend = TerrainFilters.Smoothstep(largeNoise, 0.3f, 0.2f);
            largeNoise = TerrainFilters.Smoothstep(largeNoise, 0, 0.3f);
            smallNoise = TerrainFilters.Smoothstep(smallNoise, 0, 0.3f);
